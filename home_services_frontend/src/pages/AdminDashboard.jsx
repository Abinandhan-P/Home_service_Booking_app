...
const AdminDashboard = () => {
  ...
  const handleToggleAvailability = async (serviceId, currentAvailability) => {
    try {
      ...
      toast.success(`Service marked as ${!currentAvailability ? 'available' : 'unavailable'}`);
      fetchData();
    } catch {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-black">
      <div className="max-w-7xl mx-auto py-8 px-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-secondary-600 text-lg">Manage your home services platform</p>
        </div>
        ...
        {/* Navigation Tabs */}
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-2 mb-8 border border-white/20 relative z-10">
          <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0 sm:space-x-1">
            {[
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center space-x-1 sm:space-x-2 py-2 sm:py-3 px-2 sm:px-4 rounded-xl font-medium text-sm sm:text-base transition-all duration-300 cursor-pointer ${
                  activeTab === tab.id
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-white/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <button 
                  ...
                  className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-4 rounded-xl hover:from-blue-600 hover:to-blue-700 transition-all duration-300 shadow-lg cursor-pointer"
                >
                ...
                <button 
                  ...
                  className="bg-gradient-to-r from-green-500 to-green-600 text-white p-4 rounded-xl hover:from-green-600 hover:to-green-700 transition-all duration-300 shadow-lg cursor-pointer"
                >
                  <div className="text-2xl mb-2">👨‍💼</div>
                ...
                <button 
                  ...
                  className="bg-gradient-to-r from-purple-500 to-purple-600 text-white p-4 rounded-xl hover:from-purple-600 hover:to-purple-700 transition-all duration-300 shadow-lg cursor-pointer"
                >
                  <div className="text-2xl mb-2">📊</div>
        ...
        {activeTab === 'users' && (
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-white/20" style={{ position: 'relative', zIndex: 1, backgroundColor: 'white' }}>
            <h3 className="text-2xl font-bold text-secondary-800 mb-6 font-heading">User Management</h3>
            
            {/* User role tabs */}
            <div className="mb-6" style={{ position: 'relative', zIndex: 1000 }}>
              <div className="flex space-x-2 bg-gray-100 rounded-2xl p-1">
                {['admin', 'worker', 'user'].map((role) => (
                  <button
                    key={role}
                    onClick={() => setActiveUserTab(role)}
                    className={`flex-1 py-3 px-4 rounded-xl font-medium cursor-pointer transition-all ${
                      activeUserTab === role 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" style={{ position: 'relative', zIndex: 1 }}>
              {filteredUsers.map(u => (
                <div key={u._id} className="bg-white rounded-2xl shadow-lg p-6 border border-secondary-200 hover:shadow-xl transition-all duration-300" style={{ position: 'relative', zIndex: 1 }}>
        ...
        {activeTab === 'bookings' && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-white/20" style={{ position: 'relative', zIndex: 1 }}>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
              <h3 className="text-2xl font-bold text-secondary-800 mb-4 md:mb-0 font-heading">Booking Management</h3>
              
              {/* Search and Filter */}
              <div className="flex flex-col md:flex-row gap-4" style={{ position: 'relative', zIndex: 100 }}>
                <div className="relative" style={{ zIndex: 101 }}>
                  <input
                    ...
                    style={{ position: 'relative', zIndex: 102, backgroundColor: 'white' }}
                  />
                  <svg className="w-5 h-5 text-secondary-400 absolute left-3 top-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ pointerEvents: 'none', zIndex: 103 }}>
            ...
            {/* Bookings Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6" style={{ position: 'relative', zIndex: 1 }}>
                ...
                <div key={b._id} className="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-primary-400 hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 bg-blue-100 text-blue-700 py-2 px-4 rounded-lg hover:bg-blue-200 transition-all duration-300 font-medium text-sm cursor-pointer"
                    >
                      View Details
                    </button>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-yellow-100 text-yellow-700 py-2 px-4 rounded-lg hover:bg-yellow-200 transition-all duration-300 font-medium text-sm cursor-pointer"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDeleteService(s._id)}
                        className="flex-1 bg-red-100 text-red-700 py-2 px-4 rounded-lg hover:bg-red-200 transition-all duration-300 font-medium text-sm cursor-pointer"
                      >
                        Delete
                      </button>
                    </div>
                    <button
                      onClick={() => handleToggleAvailability(s._id, s.available)}
                      className={`w-full py-2 px-4 rounded-lg transition-all duration-300 font-medium text-sm cursor-pointer ${
                        s.available 
