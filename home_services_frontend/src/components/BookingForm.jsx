              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {propertyTypes.map(type => (
                  <button
                    ...
                    className={`property-type-btn p-3 rounded-lg border-2 transition-all duration-200 cursor-pointer text-center min-h-[60px] ${bookingData.propertyType === type
            ...
            <div className="flex items-center">
              {steps.map((step, index) => (
                <React.Fragment key={step.id}>
                  <div className="flex items-center">
                    <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-semibold ${currentStep >= step.id
                        ? 'bg-blue-600 text-white'
                        : 'bg-gray-200 text-gray-600'
                      }`}>
                      {currentStep > step.id ? '✓' : step.icon}
                    </div>
                    <div className={`ml-2 text-xs font-medium hidden sm:block ${currentStep >= step.id ? 'text-blue-600' : 'text-gray-500'
                      }`}>
                      {step.title}
                    </div>
                  </div>
                  {index < steps.length - 1 && (
                    <div className={`flex-grow h-1 mx-2 sm:mx-4 ${currentStep > step.id ? 'bg-blue-600' : 'bg-gray-200'
                      }`} />
                  )}
                </React.Fragment>
              ))}
