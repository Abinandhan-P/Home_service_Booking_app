              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {propertyTypes.map(type => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setBookingData(prev => ({ ...prev, propertyType: type.value }));
                      if (errors.propertyType) setErrors(prev => ({ ...prev, propertyType: '' }));
                    }}
                    style={bookingData.propertyType === type.value
                      ? { border: '2px solid #3B82F6', background: '#EFF6FF', transform: 'scale(1.05)' }
                      : {}}
                    className={`p-6 rounded-2xl border-2 transition-all duration-300 hover:shadow-lg cursor-pointer text-left w-full ${
                  ...
                  >
                    ...
                    <div className={`text-sm mt-1 ${bookingData.propertyType === type.value ? 'text-blue-600 font-medium' : 'text-gray-500'}`}>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {timeSlots.map(slot => (
                    ...
                    >
                      {slot.label}
                      {!slot.available && <div className="text-xs mt-0.5 opacity-70">Booked</div>}
                    </button>
                  ))}
                </div>
